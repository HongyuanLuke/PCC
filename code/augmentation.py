...
#aug_file_name="aug_huff_train_10.txt"
#aug_file_name="aug_awzn_train_3.txt"
#aug_file_name="aug_amzn_train_2.txt"
aug_file_name="aug_amzn_train_s4.txt"
#aug_file_name="aug_covc_train_3.txt"
#aug_file_name="aug_fewrel_train_10.txt"
#aug_file_name="aug_fewrel_train_3.txt"
#aug_file_name="aug_covc_train_s2.txt"
#aug_file_name="aug_huff_train_s0_b.txt"
#aug_file_name="aug_covc_train_s2_b_0.txt"
#aug_file_name="aug_covc_train_s0_bottom_1.txt"


paraphrase_dict={}
with open(aug_file_name, 'r', encoding='UTF-8') as f:
    lines = f.readlines()
    for line in lines:
        w=line.split(" $ ")[0]
        if not w in paraphrase_dict:
            paraphrase_dict[w]=[line]
        else:
            paraphrase_dict[w].append(line)
...
# paraphrase
######
def get_paraphrase_sentences(train_path, n_aug, alpha):
    pkl_path = Path(train_path).parent.joinpath(f"train_aug_paraphrase_alpha{alpha:.2f}_data.pkl")
    sentence_to_aug_sentences = get_paraphrase_data_dict(pkl_path, train_path, n_aug, alpha)
    return sentence_to_aug_sentences

def get_paraphrase_data_dict(pkl_path, train_path, n_aug, alpha):
    if not pkl_path.exists():

        print(f"creating {pkl_path}")

        #sentences, _ = common.get_sentences_and_labels_from_txt(train_path)
        sentences=paraphrase_dict.keys()
        print(len(sentences))
        sentence_to_augmented_sentences = {}
        for sentence in tqdm(sentences):
            paraphrase_sentences = get_paraphrase_sentence(sentence, alpha)
            sentence_to_augmented_sentences[sentence] = paraphrase_sentences

        common.save_pickle(pkl_path, sentence_to_augmented_sentences)

    return common.load_pickle(pkl_path)

def get_paraphrase_sentence(sentence, alpha):
    ss=[]
    bleurts=[]
    #alpha=random.randint(1, 5)
    for sen in paraphrase_dict[sentence]:
        ws = sen.split(" $ ")
        bleurt = float(ws[3])
        bleurts.append(bleurt)
        ss.append(ws[1])
    ble=argsort(bleurts)
    #total_len = len(bleurts)
    if len(bleurts)>=10:
        if alpha == 1:
            b1=len(bleurts)-1
        elif alpha==2:
    ...
    if len(bleurts)<4:
        b1,b2,b3,b4=0,0,0,0
    ## these codes are used to adjust the number of augmentation
    return [ss[ble[b1]],ss[ble[b2]],ss[ble[b3]],ss[ble[b4]]]
    #return [ss[ble[b1]], ss[ble[b2]], ss[ble[b3]]]
    #return [ss[ble[b1]], ss[ble[b2]]]
    #return [ss[ble[b1]]]
...
def get_augmented_sentences(aug_type, train_path, n_aug, alpha):
    ...
    if aug_type == "sr":
        ...
        return get_paraphrase_sentences(train_path, n_aug, alpha)
    elif aug_type == "rd":
